...
tape.test('Styled component can be rendered for testing without providers', t => {
  const wrapper = enzymeMount(<TestFunctionComponent id='tfc' />);
  t.ok(wrapper.exists(), 'wrapper should exist');
  t.equal(wrapper.getDOMNode().getAttribute('id'), 'tfc', 'underlying div should have the correct ID');
  t.equal(wrapper.text(), 'Test', 'underlying div should have the correct text');
  t.end();
});